...
#' Single stage RMST test using simulation. P-value and cut off $\tau$ adjustment times
#' 
#' Different from RMST_sim_cal, It return a dataframe of p-value and rejection times of single-stage RMST test .
#' It also counts the times of $\tau$ adjustment (adjusted $\tau$ is the minimax survival time of two groups)
...
#'       \item{tau adjustment}{The proportion of simulations where $\tau$ adjustment occurred.}
#'     }
#'   }
...
#' Theoretical value of RMST (exponential distribution)
#' 
...
#' Probability of early termination using normal density
#' 
...
#' Mean and variance estimation of RMST using Monte-Carlo
#' 
...
#' 
#' This function performs a grid search to find the critical values $(m_1, m_2)$ for the log-rank test in a two-stage design.
#' It makes use of probability $P(W_1/\sigma_1 > m_1 & W_2/sigma_2 > m_2 | H_0) = \alpha$ and
...
#' When \code{power} is not given, the function finds the critical values $(m_1, m_2)$ that control the overall type I error
...
#' When \code{power} is not given, the function finds the critical values that control the overall type I error $\leq \alpha$
...
#' @param mu_cov_h0 Variance-covariance matrix of $[E_1-C_1, E_1, E_2-C_2, E_2]$ under the null hypothesis (H0), obtained from the \code{mu_cov_mc} function.
#' @param mu_cov_h1 Variance-covariance matrix of $[E_1-C_1, E_1, E_2-C_2, E_2]$ under the alternative hypothesis (H1), obtained from the \code{mu_cov_mc} function.
...
#' @param method Method used for the RMST design. "Simple" indicates a Simple RMST with the rejection region $(D_1 > m_1 & D_2 > m_2)$, where $D = E - C$ is the RMST difference between groups. "Complex" indicates a Sculpted RMST with the rejection region $(D_1 > m_1 & E_1 > q_1 & D_2 > m_2 & E_2 > q_2)$, where E is the RMST in the experimental group.
#' @param alpha Stated type I error level.
...
#' Plotting the comparison of 3 methods
#' 
